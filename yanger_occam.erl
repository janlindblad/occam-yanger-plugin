...
init(Ctx0) ->
    Ctx1 = yanger_plugin:register_output_format(Ctx0, occam, fun emit/3),
...
opts() ->
    [{"YANG output specific options:",
      [
       {occam_canonical, undefined, "occam-canonical", {boolean, false},
        "Print in canonical order"},
       {occam_comment, undefined, "occam-path-comment", {boolean, false},
...
emit(Ctx, Modules, Fd) ->
    ...
    State =
        #state{ outf = OutF,
                canonical =
                    proplists:get_value(occam_canonical, Ctx#yctx.options),
                path_comment =
                    proplists:get_value(occam_comment, Ctx#yctx.options),
                rps =
    ...
    lists:foreach(
      fun (M) ->
              ModuleMap =
                  yang:map_foldl(fun (Px, '$self', Map) ->
                                         Mod = M#module.modulename,
                                         yang:map_insert(Mod, Px, Map);
                                     (Px, Mod, Map) ->
                                         yang:map_update(Mod, Px, Map)
                                 end,
                                 yang:map_new(),
                                 M#module.prefix_map),
              State1 = State#state{
...
emit_module(M, State) ->
    %%io:format("JRL: EMIT MODULE ~p~n", [M]),
...
emit_stmts([Stmt|Stmts], Lvl, PrevKwdClass, Path0, S) ->
    OutF = S#state.outf,
    %%io:format("JRL: EMIT STMTS ~p~n", [Lvl]),
    ...
    %%io:format("JRL: KEYWORD ~p~n", [Keyword]),
    case Keyword of
        '_comment' ->
            emit_comment(yang:stmt_arg(Stmt), Lvl, S),
            NewKwdClass = PrevKwdClass;
        'must' ->
            %%io:format("JRL: MUST KEYWORD removed~n"),
            emit_comment(<<"// Occam: must-stmt removed">>, Lvl, S),
            NewKwdClass = PrevKwdClass;
        'when' ->
            %%io:format("JRL: WHEN KEYWORD removed~n"),
            emit_comment(<<"// Occam: when-stmt removed">>, Lvl, S),
            NewKwdClass = PrevKwdClass;
        _ ->
            case yang:stmt_keyword(Stmt) == 'type' andalso yang:stmt_arg(Stmt) == 'leafref' of
                'true' ->
                    emit_comment(<<"// Occam: leafref replaced">>, Lvl, S),
                    NewKwdClass = emit_stmt_keyword(Keyword, Lvl, PrevKwdClass, S),
                    emit_stmt_arg('string', Lvl, S, Keyword),
                    SubStatements = [];
                _ ->
                    NewKwdClass = emit_stmt_keyword(Keyword, Lvl, PrevKwdClass, S),
                    emit_stmt_arg(yang:stmt_arg(Stmt), Lvl, S, Keyword),
                    SubStatements = yang:stmt_substmts(Stmt)
            end,
            case SubStatements of
                [] ->
                    OutF([sc(),nl()]);
                SubStmts0 ->
                    OutF([s(),ob(),nl()]),
